package com.mietrix.quranprojects.database.datasource;

import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;

import com.mietrix.quranprojects.database.DatabaseHelper;
import com.mietrix.quranprojects.model.Tafsir;
...
public class TafsirKathirEnglishDataSource {
  ...
  public static final String TAFSIR_KATHIR_ENGLSIH_SEGMENT_TABLE =
  ...
  public ArrayList<Tafsir> getTafsirKathirEnglishArrayList(long surah_id, long verse_id) {
    ...
    cursor =
        db.rawQuery(
            "select tafsir_ibne_kathir_english.tafsir_text,tafsir_ibne_kathir_english.ayah_num,"
                + "tafsir_ibne_kathir_english.surah_num from tafsir_ibne_kathir_english \n"
                + "where tafsir_ibne_kathir_english.surah_num = "
                + surah_id
                + " and tafsir_ibne_kathir_english.ayah_num >= "
                + "(select tafsir_ibne_kathir_english_segment.ayah_begin\n"
                + "from tafsir_ibne_kathir_english_segment where  tafsir_ibne_kathir_english_segment.surah_num=  "
                + surah_id
                + " and "
                + verse_id
                + " "
                ...
                + "tafsir_ibne_kathir_english_segment \n"
                + "where  tafsir_ibne_kathir_english_segment.surah_num = "
                + surah_id
                + " and "
                + verse_id
                + " between"
                + " tafsir_ibne_kathir_english_segment.ayah_begin "
                + "and tafsir_ibne_kathir_english_segment.ayah_end)",
            null);

    cursor.moveToFirst();
    while (!cursor.isAfterLast()) {
      Tafsir tafsir = new Tafsir();
      tafsir.setTafsirText(
          cursor.getString(cursor.getColumnIndex(TAFSIR_KATHIR_ENGLSIH_TAFSIR_TEXT)));
