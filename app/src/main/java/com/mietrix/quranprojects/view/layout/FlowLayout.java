package com.mietrix.quranprojects.view.layout;
...
public class FlowLayout extends ViewGroup {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int sizeHeight =
        MeasureSpec.getSize(heightMeasureSpec) - this.getPaddingRight() - this.getPaddingLeft();
    ...
    for (int i = 0; i < count; i++) {
      ...
      child.measure(
          MeasureSpec.makeMeasureSpec(
              sizeWidth, modeWidth == MeasureSpec.EXACTLY ? MeasureSpec.AT_MOST : modeWidth),
          MeasureSpec.makeMeasureSpec(
      ...
      if (orientation == HORIZONTAL) {
        if (rtl) {
          posX = size - getPaddingRight() - lineLength;
        } else {
